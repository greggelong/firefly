...
void setup() {
  ...
  for (int j=0; j<rows; j=j+1) {
    for (int i=0; i<cols; i++) {
      //noStroke();
      grid[j][i] = new Fly(i, j, color(random(255), random(255), random(255)));
    }
  }
...
void update() {
  ...
  for (int j=0; j<rows; j=j+1) {  // dont check edges
    for (int i=0; i<cols; i++) {
      Fly myfly = grid[j][i];
      ...
      if (myfly.state ==0 && tick%myfly.chargeTime == 0) {
        ...
        switch (choice) {
        case 1:
          newfly.chargeTime = grid[(j-1+rows)%rows][(i-1+cols)%cols].chargeTime; 
          newfly.flashiness = grid[(j-1+rows)%rows][(i-1+cols)%cols].flashiness;
          newfly.flashLen = grid[(j-1+rows)%rows][(i-1+cols)%cols].flashLen;
          newfly.flashColor = grid[(j-1+rows)%rows][(i-1+cols)%cols].flashColor;
          break;
        case 2:
          newfly.chargeTime = grid[(j-1+rows)%rows][(i+cols)%cols].chargeTime; 
          newfly.flashiness = grid[(j-1+rows)%rows][(i+cols)%cols].flashiness;
          newfly.flashLen = grid[(j-1+rows)%rows][(i+cols)%cols].flashLen;
          newfly.flashColor = grid[(j-1+rows)%rows][(i+cols)%cols].flashColor;
          break;
        case 3:
          newfly.chargeTime = grid[(j-1+rows)%rows][(i+1+cols)%cols].chargeTime; 
          newfly.flashiness = grid[(j-1+rows)%rows][(i+1+cols)%cols].flashiness;
          ...
          break;
        case 4:
          newfly.chargeTime = grid[(j+rows)%rows][(i-1+cols)%cols].chargeTime; 
          newfly.flashiness = grid[(j+rows)%rows][(i-1+cols)%cols].flashiness;
          newfly.flashLen = grid[(j+rows)%rows][(i-1+cols)%cols].flashLen;
          newfly.flashColor = grid[(j+rows)%rows][(i-1+cols)%cols].flashColor;
          break;
...
int [] neighborFlash(Fly centerFly) {   // returns an array of flashing neighbors to randomly pick one
  ...
  int i = centerFly.x;
  int j = centerFly.y;
  if (grid[(j-1+rows)%rows][(i-1+cols)%cols].state == 2) {
    neighs = append(neighs, 1);
  }
  if (grid[(j-1+rows)%rows][(i+cols)%cols].state ==2) {
    neighs = append(neighs, 2);
  }
  if (grid[(j-1+rows)%rows][(i+1+cols)%cols].state ==2) {
    neighs = append(neighs, 3);
  }
  if (grid[(j+rows)%rows][(i-1+cols)%cols].state ==2) {
