class Fly {
  ...
  Fly(float tx, float ty, color tflashColor) {
  ...
  void update() {
    ...
    if (state ==0 && tick%chargeTime == 0) {
      ...
      state =2;
      // exchange trates from flasher
      // only if state is one check add condition in neighbors
  ...
  void neighborFlash(Fly vt) { // takes a fly 
    ...
    if (loc.dist(v2) < sz*3) {   // the bugs can see 1/4 the way across the field
      nFlash = true;
      // exchange trates from flasher
      // if state = 1;
      if (state ==1){
  ...
  void resetTrait(){
    ...
    flashiness =10+int(random(7));    // 10
    flashLen = 20+int(random(5));     // 3
    flashColor = clrs[int(random(clrs.length))];
